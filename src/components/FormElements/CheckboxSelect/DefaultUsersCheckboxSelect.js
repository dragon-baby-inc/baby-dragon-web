...
import { CheckboxLabel, Separater, UserCreateLabel, Warning } from "../index";
import { themeColors } from "../../../constants";

const DefaultUsersCheckboxSelect = ({
  objects,
  warning,
  selected_object_ids,
  labelsHeight,
  ...
  return (
    <div style={style ? style : {}} className={containerStyles.join(" ")}>
      ...
      {selectAll ? (
        <CheckboxLabel
          disabled={
            filterDisabled(objects).length > 0 &&
            objects.length === filterDisabled(objects).length
          }
          value="select-all"
...
export default DefaultUsersCheckboxSelect;
