...
const NewPaymentForm = ({ loading, users, afterSubmit }) => {
  ...
  const handleOwersSelectClicked = () => {
    ...
    let owers = state.owers.value
      ? state.owers.value.map((el) => el.id)
      : users.map((el) => el.id);
  ...
  const components = {
    ...
    owers: (
      <CheckboxSelectInput
        ...
        selectAll={
          state.owers.value ? state.owers.value.length === users.length : false
        }
  ...
  return (
    <div style={styles.container}>
      ...
      <Button
        disabled={disableSubmit && state.disableForm}
