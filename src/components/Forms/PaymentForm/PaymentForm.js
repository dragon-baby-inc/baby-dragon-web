...
const PaymentForm = ({
    ...
    if (!touched) {
      _setManualOwers(
        getManualOwersAmount(
          users,
          _owers.value,
          _manualOwers.value.map((o) => o.user.id),
          v
        ).state
      );
      ...
      _setOwers({
        value: users.filter((u) => validManualOwersIds.includes(u.id)),
        valid: true,
      });
    }
  };
  ...
  const getManualOwersAmount = (users, owers, unTouchedIds, totalAmount) => {
    ...
    let amount_array = split_into(
      totalAmount,
      selectedOwerIds.length,
  ...
  const handleManualOwersChanged = (index, data) => {
    ...
    let valid =
      newOwers.filter((ower) => ower.amount > 0).length === newOwers.length;
  ...
  const handleSubmit = () => {
    ...
    createPayment(authState.api, newState, (data) => {
      if (data.send_liff_confirm_message === true) {
        if (liff.isInClient()) {
          let message = buildPaymentSuccessMessage(
            data.log_message_content,
            data.log_message_category,
            data.accounting_book_name
          );
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.footer}>
          <Button clicked={handleSubmit} disabled={disabled}>
            建立帳款
          </Button>
          <div
            style={{
