...
const AccountingBookSettingPage = (props) => {
  ...
  const updateAccountingBook = (params) => {
    return authState.api.updateAccountingBook(
      accountingBookDetails.group_id,
      accountingBookDetails.id,
  ...
  const handleAccountingBookDeletion = (e) => {
    authState.api
      ...
      .then((res) => {
        history.navigate(
          `/liff_entry/groups/${accountingBookDetails.group_id}/accounting_books`
        );
  ...
  const handleCurrentChange = (value, params, setState) => {
    ...
    authState.api
      ...
      .then((res) => {
        ...
        setAutoDetectPayment(
          res.data.accounting_book.use_payment_auto_detection
        );
      })
      .catch((err) => {
        console.log(err);
  ...
  const handlInputChange = (value, params, setState) => {
    ...
    if (value && value.length > 0) {
      ...
      let updateTimer = setTimeout(() => {
        updateAccountingBook(params).then((res) => {});
      }, 2000);
          ...
          <div style={styles.settings}>
            {true ? null : (
              <label style={styles.label}>
                <span style={styles.description}>帳本名稱</span>
                {loading ? null : (
                  <input
                    style={styles.textInput}
                    type="text"
                    value={
                      state.name.value === undefined ? "" : state.name.value
                    }
                    onChange={(e) =>
                      handlInputChange(
                        e.target.value,
            ...
            <NavigationLabel
              ...
              clicked={() => {
                history.navigateTo("accountingBookCurrencyPage", {
                  group_id,
                  accounting_book_id,
                });
              }}
            ...
            <NavigationLabel
              ...
              clicked={() => {
                history.navigate(
                  `/liff_entry/groups/${accountingBookDetails.group_id}/accounting_books/${accountingBookDetails.id}/default_users`
                );
              }}
            />
            <ActionLabel
            ...
            <ToggleLabel
              ...
              changed={(e) =>
                handleCurrentChange(
                  e.target.checked,
                  { use_payment_auto_detection: e.target.checked },
                  setAutoDetectPayment
                )
              }
            ...
            <ToggleLabel
              ...
              changed={(e) =>
                handleCurrentChange(
                  e.target.checked,
                  { send_liff_confirm_message: e.target.checked },
                  setLineNotification
                )
              }
