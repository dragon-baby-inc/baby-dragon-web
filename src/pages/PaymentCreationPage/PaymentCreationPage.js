...
const PaymentCreationPage = () => {
  ...
  const { state, setPayer, setBuilder, setAccountingBookDetails, resetForm } =
  ...
  useEffect(() => {
    if (!loading && authState && authState.api) {
      ...
      if (!payer.coverCost) {
        payer = users.filter((u) => u.coverCost === true)[0];
      }
  ...
  return (
    <>
      ...
      <PageHeader color={themeColors.gray400}>
        新增帳款至
        <span style={{ color: themeColors.green700, paddingLeft: "4px" }}>
          {accountingBookDetails.name ? ` ${accountingBookDetails.name}` : ""}
        </span>
      </PageHeader>
