...
const AccountingBookUsersPage = (props) => {
  ...
  const [users, accountingBookDetails, loading, _err, setUsers] =
  ...
  const updateCoverCostUsers = () => {
    ...
    authState.api
      .updateCoverCostUsers(group_id, accounting_book_id, value)
      .then((res) => {
  ...
  const handleCreateUserConfirm = (setDisableForm) => {
    ...
    authState.api
      .createUser(group_id, { name: name.value, image_id: imageId })
      .then((res) => {
        let _users = [...users];
        _users.push({
