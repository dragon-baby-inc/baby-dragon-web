...
import { useHistory, useUsersSelect, useCurrencySelect } from "../../hooks";
import { useParams } from "react-router-dom";

import { createFreeUserCheckbokLabel } from "../../generators/labelGenerator";
...
const AccountingBookEditPage = (props) => {
  ...
  const [stepStates, setStepStates] = useState([false, true, true]);
  ...
  const handleEditUserConfirm = (setDisableForm) => {
    ...
    authState.api
      .updateUser(group_id, editObject.id, {
        name: userName.value,
        image_id: imageUserId,
      })
      .then((res) => {
        let _users = [...users];
  ...
  const handleCreateUserConfirm = (setDisableForm) => {
    ...
    authState.api
      .createUser(group_id, { name: userName.value, image_id: imageUserId })
      .then((res) => {
        let _users = [...users];
        _users.push(normalizeGroupUser(res.data.user));
        setUsers({ value: _users, valid: true });
        _setUsers(_users);
        resetConfirmBox();
  ...
  const createAccountingBook = () => {
    let formValid = validateForm(state, [
      "name",
      "users",
      "imageId",
      "currency",
    ...
    authState.api
      .createAccountingBook(group_id, buildParams())
      .then((res) => {
        history.navigateTo("paymentIndexPage", {
          group_id,
          accounting_book_id: res.data.accounting_book.id,
        });
        setDisableForm(false);
      })
      .catch((err) => {
  ...
  const steps = [
    ...
    },
    {
      name: (
        <div>
          選擇 <span style={styles.nameSpan}>{state.users.value.length}位</span>{" "}
          分帳成員
        </div>
      ),
      component: (
        <>
          <div style={styles.select}>{userSelect}</div>
        </>
      ),
    },
    {
        ...
        <div>
          ...
          <span style={styles.nameSpan}>
            {state.currency.value
              ? currencies.filter((c) => c.name === state.currency.value)[0]
                  .displayName
              : "台幣"}
          </span>{" "}
          作為帳款幣別
      ...
      component: <div style={styles.select}>{currencySelect}</div>,
    },
  ];
    ...
    <>
      <div style={styles.container}>
        <div style={styles.bg}>
          <TopRightIcon
            clicked={() => {
              history.navigateTo("accountingBookPage", { group_id });
            }}
          ...
          <div style={styles.stepName}>{steps[currentStep].name}</div>
          <StepsWidget
