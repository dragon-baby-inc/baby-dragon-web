...
const paymentReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case "set_fixed_amount":
      ...
      return {
        ...state,
        amount: { value: action.payload, valid: fixedAmountValid },
      };
    case "set_amount":
      let amountValid = action.payload > 0;
      return {
        ...state,
        amount: { value: action.payload, valid: amountValid },
      };
    case "set_creation_date":
      let dateValid = action.payload ? true : false;
      return {
        ...state,
        creation_date: { value: action.payload, valid: dateValid },
      };
    case "set_paid_back":
    ...
    case "set_payer":
      ...
      if (state.ower.value === action.payload) {
        return {
          ...state,
          payer: { value: action.payload, valid: valid },
    ...
    case "set_ower":
      ...
      if (state.payer.value === action.payload) {
        return {
          ...state,
          ower: { value: action.payload, valid: valid },
          payer: { value: null, valid: false },
        };
      } else {
    ...
    case "set_manual_owers":
      ...
      valid = action.payload.valid
        ...
        : action.payload.owers.filter((ower) => ower.amount > 0).length ===
          action.payload.owers.length;
      return {
        ...state,
        fixedAmount: { value: amount, valid: valid },
        manualOwers: { value: action.payload.owers, valid: valid },
      };
    case "set_hidden":
      return {
        ...state,
...
const setShowPopUpForm = (dispatch) => (action, objects) => {
  dispatch({
    type: "set_show_pop_up_form",
    payload: { action, objects },
  });
};
...
const validations = {
  name: ["isString", "isNotEmpty", "validPaymentName"],
...
const validateForm = (dispatch) => (state, formKeys, callback) => {
  ...
  if (formKeys.includes("manualOwers")) {
    let summaryAmount = sumOwers(
      newState["manualOwers"].value,
      state.accounting_book_details.exponent
    );
    let isKeyValid =
      parseFloat(summaryAmount) === parseFloat(newState.amount.value);

    newState["manualOwers"] = {
      value: state["manualOwers"].value,
      valid: isKeyValid,
    };
...
const createPayment = (dispatch) => (api, state, afterSubmit) => {
  ...
  if (state.paid_back) {
    ...
    if (state.allocation_type === "amount") {
      ...
      params["owers"] = state.manualOwers.value
        .filter((o) => o.amount > 0)
        .map((o) => {
  ...
  api
    .createPayment(details.group_id, details.id, params)
