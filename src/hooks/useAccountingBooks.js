...
const useAccountingBooks = (authState) => {
  ...
  const groupAccountingBooksCache = store.get(
    `groupAccountingBooks-${group_id}`
  );
  const groupCurrentBookCache = store.get(`groupCurrentBook-${group_id}`);
  const [books, setBooks] = useState(
    groupAccountingBooksCache ? groupAccountingBooksCache : []
  );
  const [currentBook, setCurrentBook] = useState(
    groupCurrentBookCache ? groupCurrentBookCache : { uuid: null }
  );
  ...
  const getAccountingBook = async () => {
    await authState.api
      ...
      .then(function (response) {
        ...
        setBooks(books);
        let currentBook = response.data.accounting_books.filter(
          (b) => b.current
        )[0];
